
# 101-linux-certificates-git

01. [SSH basics](01-ssh-without-password)
02. [SSH config file](02-using-ssh-config)
03. [User management](03-user-management)
04. [SUDO access and config](04-sudoers-file-rules)
05. [File Permissions](05-file-permissions)
06. [Using Git](06-git-fundamentals)
07. [Creating a CA](07-create-ca)
08. [Creating a CSR](08-create-csr)
09. [Validating certificates via CLI](09-openssl-view-validate-certs)
10. [Converting pk12 files](10-pk12-conversion)
11. [Installing NGINX](11-install-nginx)
12. [Installing ScribbleRS](12-scribble-rs)
